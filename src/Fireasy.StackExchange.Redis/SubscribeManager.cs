...
namespace Fireasy.Redis
{
    ...
    public class SubscribeManager : RedisComponent, ISubscribeManager
    {
            ...
            client.GetSubscriber().Subscribe(name, (channel, value) =>
                {
                    ...
                    try
                    {
                        try
                        {
                            subject = Deserialize<StoredSubject>((string)value);
                            subscriber(Deserialize<TSubject>(subject.Body));
                        }
                        catch (SerializationException)
                        {
                            subscriber(Deserialize<TSubject>((string)value));
                        }
                    }
                    catch (Exception exp)
                    {
            ...
            client.GetSubscriber().Subscribe(name, (channel, value) =>
                {
                    ...
                    try
                    {
                        ...
                        {
                            subscriber(Deserialize<TSubject>((string)value)).AsSync();
                        }
                    }
                    catch (Exception exp)
                    {
            ...
            client.GetSubscriber().Subscribe(name, (channel, value) =>
                {
                    ...
                    try
                    {
                        ...
                        {
                            subscriber.DynamicInvoke(Deserialize(subjectType, (string)value));
                        }
                    }
                    catch (Exception exp)
                    {
        ...
        private void Publish(IConnectionMultiplexer client, string name, byte[] data)
        {
            name = TopicHelper.GetTopicName(ServiceProvider, name);
        ...
        private async Task PublishAsync(IConnectionMultiplexer client, string name, byte[] data)
        {
            name = TopicHelper.GetTopicName(ServiceProvider, name);
            var pdata = new StoredSubject(name, data);
        ...
        private void PublishSubject(IConnectionMultiplexer client, StoredSubject subject)
        {
            client.GetSubscriber().Publish(subject.Name, Serialize(subject));
        }

        private async Task PublishSubjectAsync(IConnectionMultiplexer client, StoredSubject subject)
        {
            await client.GetSubscriber().PublishAsync(subject.Name, Serialize(subject)).ConfigureAwait(false);
        }
