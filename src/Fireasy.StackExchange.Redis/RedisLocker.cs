...
namespace Fireasy.Redis
{
    ...
    public class RedisLocker : RedisComponent, IDistributedLocker
    {
        ...
        {
            RedisHelper.TryLock(GetDatabase(token), token, timeout, action, onLocked);
        }

        public T TryLock<T>(string token, TimeSpan timeout, Func<T> func, Func<T> onLocked = null)
        {
            return RedisHelper.TryLock(GetDatabase(token), token, timeout, func, onLocked);
        }

        public async Task TryLockAsync(string token, TimeSpan timeout, Func<Task> func, Func<Task> onLocked = null)
        {
            await RedisHelper.TryLockAsync(GetDatabase(token), token, timeout, func, onLocked);
        }

        public async Task<T> TryLockAsync<T>(string token, TimeSpan timeout, Func<Task<T>> func, Func<Task<T>> onLocked = null)
        {
            return await RedisHelper.TryLockAsync(GetDatabase(token), token, timeout, func, onLocked);
        }
    }
}
