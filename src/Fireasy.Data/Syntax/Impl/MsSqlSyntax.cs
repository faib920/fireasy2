...
namespace Fireasy.Data.Syntax
{
    ...
    {
        private ConcurrentDictionary<string, int> _versions = new ConcurrentDictionary<string, int>();
...
{(string.IsNullOrEmpty(orderBy) ? "ORDER BY 1" : string.Empty)}{(segment.Start != null ? $" OFFSET {segment.Start - 1} ROW" : string.Empty)} FETCH NEXT {(segment.Length != 0 ? segment.Length : 1000)} ROWS ONLY";
        ...
        private int GetServerVersion(IDatabase database)
        {
            ...
            if (_versions.TryGetValue((string)database.ConnectionString, out int version))
            {
                return version;
            }
            ...
            {
                version = int.Parse(serverVersion.Split('.')[0]);
                _versions.TryAdd((string)database.ConnectionString, version);
            }
