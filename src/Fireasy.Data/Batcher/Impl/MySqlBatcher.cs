...
namespace Fireasy.Data.Batcher
{
    ...
    public sealed class MySqlBatcher : BatcherBase, IBatcherProvider
    {
        ...
        public void Insert(IDatabase database, DataTable dataTable, int batchSize = 1000, Action<int> completePercentage = null)
        {
            ...
            var batchOpt = new BatchOperator<int>(
                conn => { conn.Open(); return 1; },
                cmd => cmd.ExecuteNonQuery());

            BatchInsert(database, dataTable.Rows, dataTable.TableName, mapping, (map, command, r, item) => MapDataRow(database.Provider, map, (DataRow)item, r, command.Parameters), batchSize, completePercentage, batchOpt);
        }
        ...
        public void Insert<T>(IDatabase database, IEnumerable<T> list, string tableName, int batchSize = 1000, Action<int> completePercentage = null)
        {
            ...
            BatchInsert(database, ToCollection(list), tableName, null, (map, command, r, item) => MapListItem<T>(database.Provider, map, (T)item, r, command.Parameters), batchSize, completePercentage, batchOpt);
        }
        ...
        public async Task InsertAsync(IDatabase database, DataTable dataTable, int batchSize = 1000, Action<int> completePercentage = null, CancellationToken cancellationToken = default)
        {
            ...
            var mapping = GetNameTypeMapping(dataTable);
            var batchOpt = new BatchOperator<Task<int>>(
                conn => { conn.OpenAsync(cancellationToken); return Task.FromResult(1); },
                cmd => cmd.ExecuteNonQueryAsync(cancellationToken));

            await BatchInsert(database, dataTable.Rows, dataTable.TableName, mapping, (map, command, r, item) => MapDataRow(database.Provider, map, (DataRow)item, r, command.Parameters), batchSize, completePercentage, batchOpt);
        }
        ...
        public async Task InsertAsync<T>(IDatabase database, IEnumerable<T> list, string tableName, int batchSize = 1000, Action<int> completePercentage = null, CancellationToken cancellationToken = default)
        {
            ...
            await BatchInsert(database, ToCollection(list), tableName, null, (map, command, r, item) => MapListItem<T>(database.Provider, map, (T)item, r, command.Parameters), batchSize, completePercentage, batchOpt);
        }
