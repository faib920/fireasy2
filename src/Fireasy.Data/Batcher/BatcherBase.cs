...
namespace Fireasy.Data.Batcher
{
    ...
    internal class BatchOperator<T>
    {
        public BatchOperator(Func<DbConnection, T> openFunc, Func<DbCommand, T> executeFunc)
        {
            OpenFunc = openFunc;
            ExecuteFunc = executeFunc;
        }

        internal Func<DbConnection, T> OpenFunc { get; set; }

        internal Func<DbCommand, T> ExecuteFunc { get; set; }
    }
}
