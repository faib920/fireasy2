...
namespace Fireasy.Data.Entity.Linq.Translators
{
    ...
    public abstract class TranslatorBase : DbExpressionVisitor
    {
        ...
        protected override Expression VisitMember(MemberExpression m)
        {
            ...
            throw new TranslateException(m, new Exception(SR.GetString(SRKind.MemberTranslateNotSupported, string.Concat(m.Member.DeclaringType, ".", m.Member.Name))));
        }
        ...
        protected override Expression VisitClientJoin(ClientJoinExpression join)
        {
            ...
            Expression newProjection = new ProjectionExpression(join.Projection.Select, constructKVPair, false, join.Projection.IsNoTracking);
        ...
        protected override Expression VisitColumn(ColumnExpression column)
        {
            if (!(column is SubqueryColumnExpression sqc))
            {
                ...
                Write(GetColumnName(column.MapInfo == null ? column.Name : column.MapInfo.ColumnName));
            }
            else
            {
        ...
        protected virtual Expression VisitStringMethod(MethodCallExpression m)
        {
            switch (m.Method.Name)
            {
                ...
                case nameof(string.ToString):
                    ...
                    break;
                case nameof(StringExtension.Like):
                    Visit(m.Arguments[0]);
                    Write(" LIKE ");
                    Visit(m.Arguments[1]);
                    break;
        ...
        protected virtual string GetTableName(TableExpression table)
        {
            ...
            return Syntax.DelimitTable(tableName);
        }

        protected virtual string GetColumnName(string columnName)
        {
            return Syntax.DelimitColumn(columnName);
        }
        ...
        private string GetEscapeString(IEnumerable<Expression> exps)
        {
            foreach (var exp in exps)
            {
                if (EscapeNamedValueChecker.Check(exp))
                {
                    return " ESCAPE '/'";
                }
            }
        ...
        private class EscapeNamedValueChecker : DbExpressionVisitor
        {
            private bool _useEscape = false;

            public static bool Check(Expression expression)
            {
                var checker = new EscapeNamedValueChecker();
                checker.Visit(expression);
                return checker._useEscape;
            }

            protected override Expression VisitNamedValue(NamedValueExpression value)
            {
                if (value.UseEscape && !_useEscape)
                {
                    _useEscape = true;
                }
