...
{
    public class OleDbTranslator : TranslatorBase
    {
        public OleDbTranslator(TranslateContext transContext)
            : base(transContext)
        {
        }
        ...
        protected override Expression VisitSelect(SelectExpression select)
        {
            if (select.Skip != null)
            {
                ...
                {
                    throw new NotSupportedException("OleDb cannot support the 'skip' operation without explicit ordering");
                }
                else if (select.Take == null)
                {
                    throw new NotSupportedException("OleDb cannot support the 'skip' operation without the 'take' operation");
                }
                else
                {
                    throw new NotSupportedException("OleDb cannot support the 'skip' operation in this query");
                }
            }
